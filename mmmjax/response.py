...
def response_curves(
    ...
    *,
    ...
    quantity : str
        Observation-shaped output of ``transformed_parameters`` to sum.
        Return expected revenue or another response in the units you want
        to report. No inverse scaling is applied to this output.
    ...
    if model._data is None:
        raise ValueError("Response curves require a model with prepared data")
    ...
    if not {"media", "spend"}.issubset(inputs.values):
        raise ValueError("Response curves require paired media and spend columns")
    ...
    indices = np.array([labels.index(name) for name in selected])
    ...
    allocation = np.broadcast_to(np.asarray(totals), (1 + len(selected) * len(grid), len(labels))).copy()
    ...
    evaluate = jax.jit(lambda budgets: jax.lax.map(lambda pair: evaluator.paired_evaluation(*pair), budgets))
    ...
    response = responses[1:].reshape(len(selected), len(grid), *reference.shape).transpose(2, 3, 0, 1)
    incremental = increments[1:].reshape(len(selected), len(grid), *reference.shape).transpose(2, 3, 0, 1)
    ...
    return xr.Dataset(
        {
            ...
            "spend": (
                ("channel", "multiplier"),
                allocation[1:].reshape(len(selected), len(grid), -1)[np.arange(len(selected)), :, indices],
            ),
