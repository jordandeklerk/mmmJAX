...
2. Match every number in the prose to an output and recompute derived ones, such as differences and percentages, from the printed values. After a fit or the model changes, grep the other pages for the numbers it moved, since pages quote each other (recovery repeats checking's TV retention, and example_data carries the true ROIs and shares).
