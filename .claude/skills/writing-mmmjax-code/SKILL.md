...
- Modules live in the domain folders under `mmmjax/`, and a new module joins the folder whose work it does. Imports run one way, from `distributions/`, `media/`, and `baselines/` through `data/`, `model/`, `inference/`, and `analysis/` to `plotting/`, so a module never imports from a later folder. A folder's `__init__.py` holds only a docstring, except `distributions/`, which re-exports its families, and users reach everything through the flat `mmmjax` namespace.
