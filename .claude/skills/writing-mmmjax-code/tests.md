...
- Plot tests check what a plot draws through `plot.data`, `layer.geom.data`, and `plot.labels` on small synthetic analysis outputs, draw each plot once, and close the figure. ArviZ diagnostics check the figure in `collection.viz["figure"]`.
