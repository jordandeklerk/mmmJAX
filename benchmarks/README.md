...
Bernoulli probability and logit parameterizations alternate integer outcomes across the sample dimensions so every parameter receives both values:

```console
pixi run benchmark-distributions --profiles vector --distributions bernoulli bernoulli_logit --operations logpmf log_density value_and_grad rng
```
