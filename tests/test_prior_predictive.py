...
def scalar_model():
    ...
    return Model(parameters={"location": Real()}, log_density=forbidden_density)
...
def _prepared_model(*, scaling=False):
    ...
    def generate(key, outcome, controls, location):
        ...
        return {
            "predictive": {"prediction": mean + jax.random.normal(key, outcome.shape)},
            "log_likelihood": {"pointwise": jnp.zeros_like(outcome)},
            "log_prior": {"lp_location": -0.5 * location**2},
            "mean": mean,
            "inputs": controls,
        }
...
def test_prior_batches_preserve_seeded_draws_generated_groups_and_labels(batch_size):
    ...
    direct = {"prediction": generated["predictive"]["prediction"], "mean": generated["mean"]}
    for group, name in (("prior_predictive", "prediction"), ("prior_generated_quantities", "mean")):
        np.testing.assert_allclose(result[group][name].values[0], direct[name], rtol=2e-6, atol=2e-6)
...
def test_constrained_draws_preserve_full_parameter_shapes_and_labels():
    model = Model(
        parameters={"scale": Positive(), "fraction": Interval(0.0, 1.0), "weights": Simplex((3,))},
        log_density=lambda data, scale, fraction, weights: jnp.nan,
...
def test_unprepared_generation_receives_supplied_data():
    model = Model(
        parameters={"location": Real()},
        log_density=lambda data, location: jnp.nan,
        generated_quantities=lambda key, data, location: {"predictive": {"prediction": data["offset"] + location}},
...
def test_generation_toggle_does_not_execute_callback_and_keeps_prepared_data():
    ...
    model = Model(
        parameters={"location": Real()},
        log_density=lambda location: jnp.nan,
        generated_quantities=forbidden_generate,
...
def test_explicit_media_and_seasonality_run_for_each_prior_draw():
    ...
    def generate(key, mean, paid, annual, annual_coefficients):
        return {"predictive": {"prediction": mean}, "paid": paid, "seasonal": annual, "weights": annual_coefficients}
    ...
    model = Model(
        ...
        log_density=lambda outcome, mean: jnp.nan,
        generated_quantities=generate,
        data=data,
    ...
    for draw in range(3):
        ...
        np.testing.assert_allclose(
            result["prior_predictive"]["prediction"].values[0, draw],
            expected["predictive"]["prediction"],
            rtol=3e-6,
            atol=2e-6,
        )
...
def test_generated_aliases_inherit_parameter_axes_without_guessing_equal_shapes(declared_axes):
    model = Model(
        parameters={"coefficient": Real(dims="feature") if declared_axes else Real((2,)), "other": Real((2,))},
        log_density=lambda data, coefficient, other: jnp.nan,
...
def test_prior_draws_use_resolved_simplex_axes_without_requiring_explicit_shapes():
    model = Model(
        parameters={"weights": Simplex(dims=("region", "category"))},
        log_density=lambda data, weights: jnp.nan,
        generated_quantities=lambda key, data, weights: {"weights_copy": weights},
...
@pytest.mark.parametrize("batch_size", [1, 4, 64])
def test_prior_routes_transformed_quantities_through_generation_without_density_evaluation(batch_size):
    _, data = _prepared_model()
...
def test_prior_returns_explicit_media_and_seasonal_outputs_with_named_axes():
    ...
    model = Model(
        ...
        log_density=lambda outcome: jnp.nan,
        data=data,
        transformed_parameters=transformed,
        coords={"annual_mode": ["sin_1", "cos_1"]},
        generated_dims={
...
def test_prior_values_must_satisfy_declared_support(parameter, value):
    model = Model(parameters={"location": parameter}, log_density=lambda data, location: jnp.nan)
...
def test_support_validation_covers_later_draws(invalid, batch_size):
    model = Model(parameters={"location": Positive()}, log_density=lambda data, location: jnp.nan)
...
def test_model_coordinates_cannot_override_sample_axes(axis):
    model = Model(parameters={"location": Real()}, log_density=lambda data, location: jnp.nan, coords={axis: [10]})
...
def test_generated_dimensions_must_match_generated_arrays():
    model = Model(
        parameters={"location": Real()},
        log_density=lambda data, location: jnp.nan,
        generated_quantities=lambda key, data, location: {"predictive": {"prediction": jnp.ones(3) * location}},
        generated_dims={"prediction": ("scenario",)},
